...
struct queue {
    struct request_q req [QUEUE_SIZE];
    int head, tail;
};
...
int add_to_queue(struct request_q to_add, struct queue * the_queue)
{
	int retval = 0;
...
/* Add a new request <request> to the shared queue <the_queue> */
struct request_q get_from_queue(struct queue * the_queue)
{
	struct request_q retval;
	/* QUEUE PROTECTION INTRO START --- DO NOT TOUCH */
...
void dump_queue_status(struct queue * the_queue)
{
	...
	/* MAKE SURE NOT TO RETURN WITHOUT GOING THROUGH THE OUTRO CODE! */
    printf("Q:[");
    for (i = the_queue->head; i != the_queue->tail; i = (i + 1) % QUEUE_SIZE) {
        printf("R%llu", the_queue->req[i].req_id);
        if ((i + 1) % QUEUE_SIZE != the_queue->tail) {
            printf(",");
        }
    }
    printf("]\n");
...
int worker_main(void *arg) {
    ...
    struct request_q current_request;
    ...
    while (1) {
        ...
        server_response.req_id = current_request.req_id;
